...
// netlify/functions/stripe-webhook.js
console.log("LOADED stripe-webhook v2026-01-01-PAYFAIL-NOTIFY-PAID-ONLY+LOCK-GEOM");
...
async function cancelStripeSubscriptionSafe(subId, reason) {
  ...
  try {
    ...
    // Stripe can throw "No such subscription" if it was already canceled/deleted by another event
 ...
 * Otherwise, DB overlap guard will reject and webhook will cancel the Stripe subscription (what you’re seeing).
 */
async function upsertSubscription(sub, meta = {}) {
  ...
  const payload = {
    ...
    current_period_end: sub.current_period_end
      ...
      : null,
    // ✅ critical: only include if we have it; this is what avoids overlap conflicts
