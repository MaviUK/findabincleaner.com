  ...
  try {
    ...
    let stripeCustomerId = null;
    {
      const { data, error } = await supabase
        ...
        .limit(1);
      if (!error && data?.length && data[0].stripe_customer_id) {
    ...
    // 3) If still not found, find-by-email or create a new Stripe Customer
    if (!stripeCustomerId) {
    ...
    if (PORTAL_CONFIG_ID) {
      payload.configuration = PORTAL_CONFIG_ID; // use explicit configuration if provided
    }

    const portal = await stripe.billingPortal.sessions.create(payload);
    return json({ url: portal.url });
  } catch (e) {
    // Surface Stripe’s message so you can see problems like “No configuration provided…”
