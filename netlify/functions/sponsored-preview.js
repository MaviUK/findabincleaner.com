...
export default async (req) => {
  ...
  // categoryId used to be required, but the DB function now derives it from service_areas in some environments.
  // Keep it optional and try both RPC signatures for maximum compatibility.
  const categoryId = String(body.categoryId || body.category_id || "").trim();
  const slot = Number(body.slot ?? 1);

  if (!areaId) {
    return json(400, { ok: false, error: "Missing areaId" });
  }
  ...
  try {
    // Try newest signature first (p_area_id, p_slot), then fall back to legacy signature
    // (p_area_id, p_category_id, p_slot). This ensures UI mirrors DB availability rules.
    let data, error;
    ({ data, error } = await sb.rpc("area_remaining_preview", {
      p_area_id: areaId,
      p_slot: slot,
    }));
