...
async function ensureStripeCustomerForCleaner(cleanerId) {
  ...
  // 5) Save to cleaners for future reuse
...
export default async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "subscription",
      customer: customerId, // <-- critical: always the same customer used by Billing Portal
